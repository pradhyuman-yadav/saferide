...
import { tripApi, routeApi } from '@/api/client';
          ...
          <g key={label}>
            ...
            {count > 0 && (
              <text
                ...
                fill="#404E3B" fontFamily="DM Sans, sans-serif"
                fontWeight="500"
              >
                {count}
              </text>
...
interface BusRow {
  ...
  trips:       number;
  completed:   number;
  avgDuration: number | null;   // ms, null if no completed trips
  sosCount:    number;
  lastTripAt:  number | null;
}

function computeBusRows(trips: Trip[], buses: Bus[]): BusRow[] {
  const busMap = new Map<string, Bus>(buses.map(b => [b.id, b]));
  const rowMap = new Map<string, BusRow>();

  for (const trip of trips) {
    let row = rowMap.get(trip.busId);
    if (!row) {
      const bus = busMap.get(trip.busId);
      row = {
        busId:       trip.busId,
        regNum:      bus?.registrationNumber ?? trip.busId,
        trips:       0,
        completed:   0,
        avgDuration: null,
        sosCount:    0,
      ...
      rowMap.set(trip.busId, row);
    }

    row.trips += 1;
    if (row.lastTripAt === null || trip.startedAt > row.lastTripAt) {
      row.lastTripAt = trip.startedAt;
    }
...
export function AnalyticsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([tripApi.listTenantHistory(), routeApi.listBuses()])
      .then(([trips, bs]) => {
        if (cancelled) return;
        setAllTrips(trips);
        setBuses(bs);
  ...
  // ── Bar chart data (always last 7 days regardless of period) ─────────────

  const days7 = last7Days();
  const barDays: BarDay[] = days7.map(({ label, dayStart }) => ({
    label,
    count: allTrips.filter(t => {
      const d = startOfDay(new Date(t.startedAt));
      return d === dayStart;
    }).length,
  }));
  ...
  const busRows = computeBusRows(trips, buses);
    ...
    <div className="analytics-page">
      <div className="analytics-header">
        <div>
          ...
          <p className="analytics-subtitle">Fleet performance and trip insights.</p>
        </div>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="analytics-card">
            <div className="analytics-card-header">
              <h2 className="analytics-card-heading">Fleet breakdown</h2>
            </div>
            {busRows.length === 0 ? (
              ...
              <div className="table-scroll">
                <table className="analytics-table">
                  ...
                  <tbody>
                    {busRows.map(row => (
                      <tr key={row.busId}>
                        <td className="analytics-bus-reg">{row.regNum}</td>
                        <td>{row.trips}</td>
                        <td>
                        ...
                        return (
                          <tr key={t.id}>
                            <td className="analytics-bus-reg">
                              {bus?.registrationNumber ?? t.busId}
                            </td>
